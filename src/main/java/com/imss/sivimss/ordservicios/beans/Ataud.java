...
public class Ataud {
	...
	public DatosRequest obtenerAtaudes(Integer idVelatorio) {
		...
		.and("DATE_FORMAT(STP.TIM_ALTA,'YY-MM-DD')=DATE_FORMAT(CURRENT_DATE(),'YY-MM-DD')")
		.and("TIMESTAMPDIFF(MINUTE,DATE_ADD(STP.TIM_ALTA, INTERVAL 4 HOUR),CURRENT_TIMESTAMP()) <= 0");
		
		selectQueryUtilInventario.select("IFNULL(SDCP.ID_INVE_ARTICULO,0)")
		.from("SVC_DETALLE_CARAC_PRESUP SDCP")
		.where("SDCP.IND_ACTIVO=1");
		
		selectQueryUtilArticulo.select("DISTINCT STA.ID_ARTICULO AS idArticulo","STI.ID_INVE_ARTICULO AS idInventario","CONCAT(CVE_FOLIO_ARTICULO,'-',STA.REF_MODELO_ARTICULO,' $',SCA.MON_PRECIO) AS nombreArticulo",
		...
		.where("STI.IND_ESTATUS NOT IN (1,2)")
