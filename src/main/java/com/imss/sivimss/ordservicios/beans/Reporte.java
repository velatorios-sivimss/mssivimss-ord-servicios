...
public class Reporte {
	...
	public DatosRequest consultarContratoServicioInmediatoTemp(DatosRequest request, ContratoServicioInmediatoRequest contratoServicioInmediatoRequest) {
		...
		queryUtil.select("SOE.CVE_FOLIO as folioOds","CONCAT_WS(' ',SP.NOM_PERSONA,SP.NOM_PRIMER_APELLIDO,SP.NOM_SEGUNDO_APELLIDO ) AS  nombreContratante",
				"CONCAT_WS(' ',NULLIF(SD.REF_CALLE,''), NULLIF(SD.NUM_EXTERIOR ,''),NULLIF(SD.NUM_INTERIOR ,''), CONCAT_WS(', ',NULLIF(SD.REF_COLONIA,' '),NULLIF(SD.REF_CP,' '),NULLIF(SD.REF_MUNICIPIO,' '),NULLIF(SD.REF_ESTADO,' ')) ) AS direccionCliente",
				"SP.CVE_RFC AS rfcCliente","SP.REF_TELEFONO AS telefonoCliente","SP.REF_CORREO  AS correoCliente","CONCAT_WS(' ',NULLIF(SDV.REF_CALLE,''), NULLIF(SDV.NUM_EXTERIOR ,''),NULLIF(SDV.NUM_INTERIOR ,''), CONCAT_WS(', ',NULLIF(SDV.REF_COLONIA,' '),NULLIF(SDV.REF_CP,' '),NULLIF(SDV.REF_MUNICIPIO,' '),NULLIF(SDV.REF_ESTADO,' ')) ) AS direccionVelatorio",
				"NULLIF(SCA.DES_CAPILLA,'') AS capillaVelatorio","date_format(SOE.FEC_ALTA,'%d/%m/%Y') AS fechaOds",
				"DATE_FORMAT(SIS.TIM_HORA_RECOGER, '%H:%i') AS horarioInicio","DATE_FORMAT(SISV.FEC_INSTALACION ,'%d/%m/%Y') AS fechaServicio","DATE_FORMAT(SISV.TIM_HORA_VELACION, '%H:%i') AS horarioFin",
				...
				"DATE_FORMAT(SIS.TIM_HORA_CREMACION, '%H:%i') AS horarioServicio","(SELECT GROUP_CONCAT(SS.DES_SERVICIO SEPARATOR ', ') FROM SVC_ORDEN_SERVICIO SOE INNER JOIN SVC_CARAC_PRESUP_TEMP SCP ON SCP.ID_ORDEN_SERVICIO = SOE.ID_ORDEN_SERVICIO AND SCP.IND_ACTIVO = 1 INNER JOIN SVC_DETALLE_CARAC_PRESUP_TEMP SDCP ON SDCP.ID_CARAC_PRESUPUESTO = SCP.ID_CARAC_PRESUPUESTO AND SDCP.IND_ACTIVO = 1 "
				...
				"SPN.REF_PANTEON AS nombrePanteon","SCP.CAN_PRESUPUESTO AS totalOds","(SELECT SPS.TIP_PARAMETRO FROM SVC_PARAMETRO_SISTEMA SPS WHERE SPS.DES_PARAMETRO = 'NOMBRE FIBESO' AND SPS.ID_FUNCIONALIDAD = 20) AS nombreFibeso","(SELECT SPS.TIP_PARAMETRO FROM SVC_PARAMETRO_SISTEMA SPS WHERE SPS.DES_PARAMETRO = 'CORREO FIBESO' AND SPS.ID_FUNCIONALIDAD = 20) AS correoFibeso",
				"(SELECT SPS.TIP_PARAMETRO FROM SVC_PARAMETRO_SISTEMA SPS WHERE SPS.DES_PARAMETRO = 'TELEFONO QUEJAS' AND SPS.ID_FUNCIONALIDAD = 20) AS telefonoQuejas")
		...
		.leftJoin("SVT_DISPONIBILIDAD_CAPILLAS SDC", "SDC.ID_ORDEN_SERVICIO = SOE.ID_ORDEN_SERVICIO").and("SDC.IND_ACTIVO = 1")
		.leftJoin("SVC_CAPILLA SCA", "SCA.ID_CAPILLA  = SDC.ID_CAPILLA")
	...
	public DatosRequest consultarContratoServicioInmediato(DatosRequest request, ContratoServicioInmediatoRequest contratoServicioInmediatoRequest) {
		...
		queryUtil.select("SOE.CVE_FOLIO as folioOds","CONCAT_WS(' ',SP.NOM_PERSONA,SP.NOM_PRIMER_APELLIDO,SP.NOM_SEGUNDO_APELLIDO ) AS  nombreContratante",
				...
				"DATE_FORMAT(SIS.TIM_HORA_RECOGER, '%H:%i') AS horarioInicio","DATE_FORMAT(SISV.FEC_INSTALACION ,'%d/%m/%Y') AS fechaServicio","DATE_FORMAT(SISV.TIM_HORA_VELACION, '%H:%i') AS horarioFin",
				"CONCAT_WS(' ',SPA.NOM_PERSONA,SPA.NOM_PRIMER_APELLIDO,SPA.NOM_SEGUNDO_APELLIDO ) AS nombreFinado","SDL.DES_DELEGACION AS lugarExpedicion",
				"DATE_FORMAT(SIS.TIM_HORA_CREMACION, '%H:%i') AS horarioServicio","(SELECT GROUP_CONCAT(SS.DES_SERVICIO SEPARATOR ', ') FROM SVC_ORDEN_SERVICIO SOE INNER JOIN SVC_CARAC_PRESUPUESTO SCP ON SCP.ID_ORDEN_SERVICIO = SOE.ID_ORDEN_SERVICIO AND SCP.IND_ACTIVO = 1 INNER JOIN SVC_DETALLE_CARAC_PRESUP SDCP ON SDCP.ID_CARAC_PRESUPUESTO = SCP.ID_CARAC_PRESUPUESTO AND SDCP.IND_ACTIVO = 1 "
						.concat("LEFT JOIN SVT_SERVICIO SS ON SDCP.ID_SERVICIO = SS.ID_SERVICIO WHERE SOE.ID_ORDEN_SERVICIO = "+ contratoServicioInmediatoRequest.getIdOrdenServicio()+") AS descripcionServicio"),
				"SPN.REF_PANTEON AS nombrePanteon","SCP.CAN_PRESUPUESTO AS totalOds","(SELECT SPS.TIP_PARAMETRO FROM SVC_PARAMETRO_SISTEMA SPS WHERE SPS.DES_PARAMETRO = 'NOMBRE FIBESO' AND SPS.ID_FUNCIONALIDAD = 20) AS nombreFibeso","(SELECT SPS.TIP_PARAMETRO FROM SVC_PARAMETRO_SISTEMA SPS WHERE SPS.DES_PARAMETRO = 'CORREO FIBESO' AND SPS.ID_FUNCIONALIDAD = 20) AS correoFibeso",
				"(SELECT SPS.TIP_PARAMETRO FROM SVC_PARAMETRO_SISTEMA SPS WHERE SPS.DES_PARAMETRO = 'TELEFONO QUEJAS' AND SPS.ID_FUNCIONALIDAD = 20) AS telefonoQuejas")
		.from("SVC_ORDEN_SERVICIO SOE")
