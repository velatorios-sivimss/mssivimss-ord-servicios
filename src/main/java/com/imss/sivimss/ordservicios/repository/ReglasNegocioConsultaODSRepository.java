...
public class ReglasNegocioConsultaODSRepository {
	...
	public String obtenerContratante() {
		...
		selectQueryUtil.select(
				"sc.ID_CONTRATANTE AS idContratante, sc.CVE_MATRICULA AS matricula, sp.CVE_RFC AS rfc, sp.CVE_CURP AS curp"
	...
	public String obtenerNombreContratante() {
		SelectQueryUtil selectQueryUtil = new SelectQueryUtil();
		selectQueryUtil.select(
				"DISTINCT CONCAT(IFNULL(sp.NOM_PERSONA,''), ' ', IFNULL(sp.NOM_PRIMER_APELLIDO,''), ' ', IFNULL(sp.NOM_SEGUNDO_APELLIDO,'')) AS nombreCompletoContratante")
				.from(TABLA_SVC_PERSONA_SP)
	...
	public String obtenerFinado() {
		SelectQueryUtil selectQueryUtil = new SelectQueryUtil();
		selectQueryUtil.select(
				"sf.id_finado AS idFinado, CONCAT(sp.NOM_PERSONA, ' ', sp.NOM_PRIMER_APELLIDO, ' ', sp.NOM_SEGUNDO_APELLIDO) AS nombreCompletoFinado"
	...
	public String obtenerNombreFinado() {
		SelectQueryUtil selectQueryUtil = new SelectQueryUtil();
		selectQueryUtil.select(
				"DISTINCT CONCAT(IFNULL(sp.NOM_PERSONA,''), ' ', IFNULL(sp.NOM_PRIMER_APELLIDO,''), ' ', IFNULL(sp.NOM_SEGUNDO_APELLIDO,'')) AS nombreCompletoFinado")
				.from(TABLA_SVC_PERSONA_SP)
		...
		selectQueryUtil.select("DISTINCT(sos.CVE_FOLIO) AS noFolioODS", "DATE_FORMAT(sos.FEC_ALTA,'%d-%m-%Y') as fecha",
				...
				.join("SVT_INVENTARIO_ARTICULO sia",
						"ID_INVE_ARTICULO  = (SELECT sdcp.ID_INVE_ARTICULO FROM SVC_CARAC_PRESUPUESTO scp "
								+ JOIN
								+ "  SVC_DETALLE_CARAC_PRESUP sdcp ON sdcp.ID_CARAC_PRESUPUESTO = scp.ID_CARAC_PRESUPUESTO "
								+ JOIN
								+ "  SVC_ORDEN_SERVICIO sos ON sos.ID_ORDEN_SERVICIO = scp.ID_ORDEN_SERVICIO WHERE sdcp.ID_INVE_ARTICULO IS NOT NULL AND sos.ID_ORDEN_SERVICIO  = "
								+ idOrdenServicio + ")")
	...
	public String obtenerODS(ReporteDto reporteDto) {
		String str = // "SELECT * FROM ("
				queryODSGen(reporteDto) + " UNION " + queryODSPreOrden(reporteDto)
		// + " GROUP BY TEMP.idOrdenServicio "
		;
		log.info(str);
		return str;
	}
	...
	private String queryODSPreOrden(ReporteDto reporteDto) {
		String str = queryCamposConsultaODSTemp()
				...
				+ JOIN
				+ " SVC_CARAC_PRESUP_TEMP scpt ON scpt.ID_ORDEN_SERVICIO = sos.ID_ORDEN_SERVICIO   "
				...
				+ LEFT_JOIN
				+ " SVC_UNIDAD_MEDICA sum2 ON sum2.ID_UNIDAD_MEDICA = IFNULL(sf.ID_UNIDAD_PROCEDENCIA, sf.ID_CLINICA_ADSCRIPCION) "
				+ LEFT_JOIN + " SVT_CONVENIO_PF scp ON scp.ID_CONVENIO_PF = sf.ID_CONTRATO_PREVISION "
				+ JOIN
				+ " SVC_ESTATUS_ORDEN_SERVICIO seos ON seos.ID_ESTATUS_ORDEN_SERVICIO = sos.ID_ESTATUS_ORDEN_SERVICIO "
				+ LEFT_JOIN
				+ " SVC_DETALLE_CARAC_PRESUP_TEMP sdcpt ON scpt.ID_CARAC_PRESUPUESTO = sdcpt.ID_CARAC_PRESUPUESTO AND sdcpt.IND_ACTIVO = 1"
				+ LEFT_JOIN
				+ " SVT_INVENTARIO_ARTICULO sia ON sdcpt.ID_INVE_ARTICULO = sia.ID_INVE_ARTICULO AND sia.ID_TIPO_ASIGNACION_ART IN (3, 4) "
				...
				+ LEFT_JOIN
				+ " SVC_SALIDA_DONACION_TEMP ssdt ON ssdt.ID_ORDEN_SERVICIO = sos.ID_ORDEN_SERVICIO AND ssdt.IND_ACTIVO = 1 "
				+ LEFT_JOIN
				+ " SVC_SALIDA_DON_ATAUDES_TEMP ssdat ON ssdat.ID_SALIDA_DONACION  = ssdt.ID_SALIDA_DONACION "
				+ LEFT_JOIN + " SVT_PLAN_SFPA sps ON sps.ID_PLAN_SFPA = sf.ID_CONTRATO_PREVISION_PA ";
		str = str + generaReporteConsultaODSPreorden(reporteDto);
		return str;
	}
	...
	private String queryODSGen(ReporteDto reporteDto) {
		String str = queryCamposConsultaODS()
				...
				+ " SVC_ESTATUS_ORDEN_SERVICIO seos ON seos.ID_ESTATUS_ORDEN_SERVICIO = sos.ID_ESTATUS_ORDEN_SERVICIO "
				+ LEFT_JOIN
				+ " SVC_DETALLE_CARAC_PRESUP sdcp ON scp2.ID_CARAC_PRESUPUESTO = sdcp.ID_CARAC_PRESUPUESTO AND sdcp.IND_ACTIVO = 1"
				+ LEFT_JOIN
				+ " SVT_INVENTARIO_ARTICULO sia ON sdcp.ID_INVE_ARTICULO = sia.ID_INVE_ARTICULO AND sia.ID_TIPO_ASIGNACION_ART IN (3,4) "
				+ LEFT_JOIN + " SVC_ATAUDES_DONADOS sad ON sad.ID_INVE_ARTICULO = sia.ID_INVE_ARTICULO "
	...
	public String actualizaOperadorODS(OperadorRequest operadorRequest, UsuarioDto usuario) {
		String str = "UPDATE SVC_ORDEN_SERVICIO SET  ID_OPERADOR = " + operadorRequest.getIdOperador()
				+ ", ID_USUARIO_MODIFICA = " + usuario.getIdUsuario() + ", " + SET_CAMPO_FEC_MODIFICA
				+ WHERE_ID_ORDEN_SERVICIO + operadorRequest.getIdOrdenServicio();
		log.info(str);
		return str;
	}
	...
	public String cancelarODS(CancelacionODSDto idODS, UsuarioDto usuario) {
		String str = "UPDATE SVC_ORDEN_SERVICIO SET  ID_ESTATUS_ORDEN_SERVICIO = 0, DES_MOTIVO_CANCELACION ='"
				+ idODS.getMotivoCancelacion() + "'"
				+ ", ID_USUARIO_MODIFICA = " + usuario.getIdUsuario() + ", " + SET_CAMPO_FEC_BAJA
				+ WHERE_ID_ORDEN_SERVICIO + idODS.getIdOrdenServicio();
		log.info(str);
		return str;
	}

	public String cancelarCaracteristicasPaquete(Integer idODS, UsuarioDto usuario) {
		String str = "UPDATE SVC_CARACTERISTICAS_PAQUETE SET  IND_ACTIVO = 0, ID_USUARIO_MODIFICA = "
	...
	public String cancelarDetalleCaracteristicasPaquete(Integer idODS, UsuarioDto usuario) {
		String str = "UPDATE SVC_DETALLE_CARAC_PAQ SET  IND_ACTIVO = 0, ID_USUARIO_MODIFICA = " + usuario.getIdUsuario()
				+ ", "
				+ SET_CAMPO_FEC_BAJA
				+ " WHERE ID_DETALLE_CARAC IN (SELECT ID_CARAC_PAQUETE FROM SVC_CARACTERISTICAS_PAQUETE "
				+ WHERE_ID_ORDEN_SERVICIO + idODS + ")";
		log.info(str);
		return str;
	}
	...
	public String cancelarDetalleCaracteristicasPresupuesto(Integer idODS, UsuarioDto usuario) {
		String str = "UPDATE SVC_DETALLE_CARAC_PRESUP SET  IND_ACTIVO = 0, ID_USUARIO_MODIFICA = "
				+ usuario.getIdUsuario() + ", "
				+ SET_CAMPO_FEC_BAJA
				+ " WHERE ID_CARAC_PRESUPUESTO IN (SELECT ID_CARAC_PRESUPUESTO  FROM SVC_CARAC_PRESUPUESTO "
				+ WHERE_ID_ORDEN_SERVICIO + idODS + ") ";
		log.info(str);
		return str;
	}
	...
	public String cancelarInventarioArticulo(Integer idODS, UsuarioDto usuario) {
		String str = "UPDATE SVT_INVENTARIO_ARTICULO SET ID_TIPO_ASIGNACION_ART=3, IND_ESTATUS = 0, ID_USUARIO_MODIFICA = "
				+ usuario.getIdUsuario() + ", " + SET_CAMPO_FEC_BAJA
	...
	public String cancelarSalidaDonacion(Integer idODS, UsuarioDto usuario) {
		String str = "UPDATE SVC_SALIDA_DONACION SET IND_ACTIVO = 0, ID_USUARIO_MODIFICA = " + usuario.getIdUsuario()
				+ ", " + SET_CAMPO_FEC_BAJA + WHERE_ID_ORDEN_SERVICIO + idODS;
		log.info(str);
		return str;
	}
	...
	public String actualizaCostoCancelacionPagoBitacora(CancelacionODSDto cancelacionODS, UsuarioDto usuario) {
		String str = "UPDATE SVT_PAGO_BITACORA SET IMP_VALOR = " + cancelacionODS.getCostoCancelacion()
				+ ", CVE_ESTATUS_PAGO = 2, ID_USUARIO_MODIFICA = " + usuario.getIdUsuario() + ", "
				+ SET_CAMPO_FEC_MODIFICA
				+ " WHERE ID_FLUJO_PAGOS = 1 AND ID_REGISTRO = " + cancelacionODS.getIdOrdenServicio()
				+ " AND CVE_FOLIO = '" + cancelacionODS.getNumeroFolio() + "'";
		log.info(str);
		return str;
	}
	...
	public String generaReporteConsultaODS(ReporteDto reporteDto) {
		...
		if (reporteDto.getIdEstatusODS() != null) {
			...
			condicion = " WHERE "
					+ "CONCAT(IFNULL(sp.NOM_PERSONA,''), ' ', IFNULL(sp.NOM_PRIMER_APELLIDO,''), ' ', IFNULL(sp.NOM_SEGUNDO_APELLIDO,'')) LIKE '%"
					+ reporteDto.getNombreContratante() + "%' ";
			if (reporteDto.getNombreFinado() != null) {
				condicion = condicion + AND_NOMBRE_PERSONA2 + reporteDto.getNombreFinado() + "%' ";
			}
			if (reporteDto.getIdTipoODS() != null)
	...
	// Bloque Generacion Reportes
	public String generaReporteConsultaODSPreorden(ReporteDto reporteDto) {
		String condicion = "";
		if (reporteDto.getIdEstatusODS() != null) {
			condicion = " WHERE sos.ID_ESTATUS_ORDEN_SERVICIO = " + reporteDto.getIdEstatusODS();
			if (reporteDto.getIdVelatorio() != null)
